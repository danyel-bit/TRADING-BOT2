...
def split_by_regime(bars, ma_period=50):
    """Splits history into bullish and bearish stretches using the ticker's own
    50-day trend as a proxy for market condition.

    Why this matters: a strategy that only looks good because the whole test
    window happened to trend upward is fragile in a way a single blended number
    completely hides. Testing each regime separately exposes that.
    """
    if len(bars) < ma_period + 10:
        return None, None
    bull, bear = [], []
    for i in range(ma_period, len(bars)):
        window = bars[i-ma_period:i]
        ma = sum(b["c"] for b in window) / ma_period
        (bull if bars[i]["c"] > ma else bear).append(bars[i])
    return bull, bear
...
def backtest_symbol(symbol):
    ...
    bull_bars, bear_bars = split_by_regime(bars)

    best = None
    for m in MOMENTUM_GRID:
        for s in STOP_GRID:
            for tp in PROFIT_GRID:
                result = score(simulate(bars, m, s, tp))
                if result is None:
                    continue

                # Check the same parameters separately in up-trending and
                # down-trending stretches. Params that only work in one are
                # penalized, because a live market will hand you both.
                bull_r = score(simulate(bull_bars, m, s, tp)) if bull_bars else None
                bear_r = score(simulate(bear_bars, m, s, tp)) if bear_bars else None

                robustness = 1.0
                regime_note = "not enough data to split by regime"
                if bull_r and bear_r:
                    both_positive = bull_r["total_return_pct"] > 0 and bear_r["total_return_pct"] > 0
                    robustness = 1.25 if both_positive else 0.6
                    regime_note = (f"bull {bull_r['total_return_pct']:+.0f}% / "
                                   f"bear {bear_r['total_return_pct']:+.0f}%"
                                   + (" — holds up in both" if both_positive else " — only works in one regime"))
                elif bull_r:
                    robustness = 0.85
                    regime_note = f"bull-only sample ({bull_r['total_return_pct']:+.0f}%)"

                adjusted = result["total_return_pct"] * robustness
                if best is None or adjusted > best["adjusted"]:
                    best = {
                        "params": {"buy_momentum_pct": m, "stop_loss_pct": s, "take_profit_pct": tp},
                        "result": result, "adjusted": adjusted, "regime_note": regime_note,
                    }
    ...
    note = (
        f"{symbol}: momentum +{p['buy_momentum_pct']}%, stop {p['stop_loss_pct']}%, "
        f"take-profit +{p['take_profit_pct']}% — {r['n_trades']} trades, "
        f"{r['win_rate']*100:.0f}% win rate, {r['total_return_pct']:+.1f}% return "
        f"[{best['regime_note']}]."
    )
...
def main():
    ...
    for symbol in WATCHLIST:
        ...
        # If the morning review has already tuned this ticker from real live
        # trades, that's earned, ongoing learning — a monthly backtest re-run
        # must never silently overwrite it with a fresh historical guess.
