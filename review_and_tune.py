...
def market_move_on(date_str):
    """How far SPY moved on a given day — used to tell apart 'the whole market
    fell' from 'this specific stock failed'."""
    try:
        r = requests.get(
            f"{DATA_BASE}/v2/stocks/SPY/bars",
            headers=HEADERS,
            params={"timeframe": "1Day", "start": date_str, "limit": 2, "feed": "iex"},
            timeout=20,
        )
        r.raise_for_status()
...
def categorize_losses(trades):
    """Groups losing trades by WHY they lost, not just that they did.

    This is the difference between "AAPL lost 6%, tighten the stop" and
    "AAPL lost 6% on a day the whole market dropped 3% — that's a market
    problem, not a signal problem, and tightening the stop wouldn't have
    helped." A human trader makes this distinction instinctively; the bot
    now makes it explicitly.
    """
    losses = [t for t in trades if t["pct"] <= 0]
    if not losses:
        return [], {}

    buckets = {"market-driven": [], "stock-specific": [], "unknown": []}
    for t in losses:
        day = (t.get("closed") or "")[:10]
        spy_move = market_move_on(day) if day else None
        if spy_move is None:
            buckets["unknown"].append(t)
        elif spy_move <= -1.0:
            buckets["market-driven"].append(t)
        else:
            buckets["stock-specific"].append(t)

    notes = []
    if buckets["market-driven"]:
        n = len(buckets["market-driven"])
        notes.append(
            f"{n} of {len(losses)} losses happened on days the broad market fell 1%+ — "
            f"those are market-driven, not signal failures. Tightening entry rules wouldn't have prevented them; "
            f"the regime filter is the right defense there."
        )
    if buckets["stock-specific"]:
        n = len(buckets["stock-specific"])
        syms = ", ".join(sorted({t["symbol"] for t in buckets["stock-specific"]}))
        notes.append(
            f"{n} of {len(losses)} losses were stock-specific (market was fine) — these are real signal failures "
            f"worth tuning against. Affected: {syms}."
        )
    if buckets["unknown"]:
        notes.append(f"{len(buckets['unknown'])} loss(es) couldn't be categorized (missing market data for that day).")
...
def main():
    ...
    if overall_stats is None or overall_stats["n"] < MIN_TRADES_TO_TUNE:
        ...
        # Understand WHY the losses happened before reacting to them.
        cause_notes, buckets = categorize_losses(trades)
        all_notes += cause_notes

        # Tune against the trades that actually reflect signal quality. A loss
        # caused by the entire market dropping says nothing about whether the
        # entry rule was good, so it shouldn't drag the parameters around.
        market_driven = len(buckets.get("market-driven", []))
        if market_driven and market_driven >= overall_stats["losses"] * 0.5:
            tuning_trades = [t for t in trades if t not in buckets.get("market-driven", [])]
            tuning_stats = stats_for(tuning_trades)
            if tuning_stats and tuning_stats["n"] >= MIN_TRADES_TO_TUNE:
                all_notes.append("Most losses were market-driven — tuning against the remaining signal-driven trades only.")
                new_global, notes = tune_params(state["global"], tuning_stats, "Base strategy")
            else:
                all_notes.append("Most losses were market-driven and too few signal-driven trades remain to tune on — leaving parameters alone.")
                new_global, notes = state["global"], []
        else:
